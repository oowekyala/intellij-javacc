...
object PsiHierarchyAdapter : TreeLikeAdapter<PsiElement> {
    override fun getChild(node: PsiElement, i: Int): PsiElement = node.children[i]

    override fun numChildren(node: PsiElement): Int = node.children.size

    override fun nodeName(type: Class<out PsiElement>): String =
            type.simpleName.removeSurrounding("Jcc", "Impl")

    override fun childIndex(node: PsiElement): Int = node.siblingSequence(forward = false).count() - 1

    override fun parent(node: PsiElement): PsiElement? = node.parent
}

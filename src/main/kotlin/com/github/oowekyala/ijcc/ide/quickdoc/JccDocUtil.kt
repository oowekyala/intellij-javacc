...
object JccDocUtil {
    ...
    fun findLinkTarget(psiManager: PsiManager?, ref: String?, context: PsiElement?): PsiElement? {
        ...
        if (ref.contains('/')) {
            ...
            if (type == LexStateRef && name == LexicalState.DefaultStateName) {
                return psiFile.lexicalGrammar.getLexicalState(name)?.declarationIdent
                    ?: psiFile.fakeDefaultStateDecl
            }

            return when (type) {
                TerminalRef    -> psiFile.globalNamedTokens
                LexStateRef    -> psiFile.lexicalStatesFirstMention
                NonterminalRef -> psiFile.nonTerminalProductions
                else           -> emptySequence<JccPsiElement>()
